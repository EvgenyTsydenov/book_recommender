...
from models.memory_based import RecommenderII
from optuna_utils import OptunaObjective


class ObjectiveMemBased(OptunaObjective):
    """Objective to minimize for hyperparameter tuning.

    This tunes collaborative filtering recommender
    that uses the user-based approach.
    ...
    def __init__(self, train_data_path: str, val_data_path: str,
        ...
        self._source_files = ['../models/memory_based.py',
                              '../models/mixins.py', 'memory_based.py']
        self._tags = ['collaborative filtering', 'memory-based', 'item-item']
        self._description = 'Optuna tunes hyperparameters of item-based ' \
                            'collaborative filtering recommender.'
        self._name = 'Item-based recommender'
    ...
    def __call__(self, trial: optuna.trial.Trial) -> float:
        ...
        model_param = {
            ...
            'min_sim_threshold': trial.suggest_float(
                'min_sim_threshold', low=1e-5, high=2e-1, log=True),
        }
        overlaps_strategy = trial.suggest_categorical(
            'overlaps_strategy', [None, 'min_overlaps', 'overlaps_penalty'])
        if overlaps_strategy == 'min_overlaps':
            model_param['min_overlaps'] = \
                trial.suggest_int('min_overlaps', 2, 5)
        elif overlaps_strategy == 'overlaps_penalty':
            model_param['overlaps_penalty'] = \
                trial.suggest_int('overlaps_penalty', 2, 5)
        ...
        try:
            # Create model
            model = RecommenderII(**model_param)

            # Train
            start_time = time.time()
            model.fit(x=(self._train_data['user_id'],
            ...
            # Evaluate
            train_predict = model.predict((self._train_data['user_id'],
                                           self._train_data['work_id']))
            val_predict = model.predict((self._val_data['user_id'],
